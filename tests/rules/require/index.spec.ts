...
import { createJsOption, createOption, createTsOption } from '../../test-utils/test'
...
ruleTester.run('require/js-simple-path case', requireLimitation, {
  ...
  invalid: [
    ...
    `,
      errors: [
        { message: createErrorMessage('./js/require/forbid', 'require') },
      ],
    },
  ],
})
...
ruleTester.run('require/ts-simple-path case', requireLimitation, {
  ...
  invalid: [
    ...
    `,
      errors: [
        { message: createErrorMessage('./ts/require/forbid', 'require') },
      ],
    },
  ],
})
...
ruleTester.run(
  'require/*.(test|spec)(.js|.ts) ignore patterning',
  // @ts-ignore
  requireLimitation,
  {
    valid: [
